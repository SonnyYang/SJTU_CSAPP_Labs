...
#1.add the "iaddl" operation
#    this can reduce codes combination like: "irmovl $1, %edi + addl %edi, %eax" into one operation: "iaddl $1, %eax"

#2.eliminate the l/u hazard of "mrmovl + andl/pushl" by forwarding the value m_valM to e_valA

#3.reduce one bubble in mis-prediction of the combination of "rmmovl + jle"
#    I change the ifun of rmmovl to "1"(that is the ifun of jle), and when rmmovl enter the Memory stage, I can get Cnd and compare it with jle in Execute stage ,than fetch the right code in Fetch stage. than there will be only one bubble in this combination

#4.As for the combination of "mrmovl jmpTable(%edx), %edx" + "mrmovl (%ebx), %edi" + "pushl %edx" + "ret"
#    when "ret" enter the fetch stage, I detect the icode in D,E,M stage ,and when the codes are exactly this combination, I change the ifun of "ret" into "1" to show that this "ret" is special, and I intsig D_ifun and W_ifun to help myself.I didn't change the upper code, I just add something under it. When the special ret with ifun "1" enter the Decode stage, we can forward the W_valM to f_pc, thus reduce three bubbles.
...
int f_pc = [
	D_icode == 9 && E_icode == IPUSHL && M_icode == IMRMOVL && W_icode == IMRMOVL && W_dstM == E_srcA && M_dstM != E_srcA : W_valM;
	E_icode == IJXX && E_ifun == 1 && M_icode == IRMMOVL && !M_Cnd : E_valA;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !(M_ifun == 1 && W_icode == IRMMOVL) && !M_Cnd : M_valA;
	# Completion of RET instruction.
	W_icode == IRET && W_ifun == 0 : W_valM;
...
# Determine ifun
int f_ifun = [
	f_icode == IRET && D_icode == IPUSHL && E_icode == IMRMOVL && M_icode == IMRMOVL && M_dstM == D_rA && E_dstM != D_rA : 1;
	f_icode == IRMMOVL : 1;
	imem_error : FNONE;
	1: imem_ifun;
];
...
int aluA = [
	E_icode == IOPL && M_icode == IMRMOVL  && M_dstM == E_srcA : m_valM;
...
int aluB = [
	E_icode == IOPL && M_icode == IMRMOVL && M_dstM == E_srcB : m_valM;
...
int e_valA = [
	E_icode == IPUSHL && M_icode == IMRMOVL && M_dstM == E_srcA : m_valM;
	1 : E_valA;
...
bool F_stall =
	...
	(E_icode == IPOPL || !(D_icode in {IOPL, IPUSHL}) && E_icode == IMRMOVL)&&
	 E_dstM in { d_srcA, d_srcB } ||
	# Stalling at fetch while ret passes through pipeline
	((D_icode == IRET && D_ifun == 0) || (E_icode == IRET && E_ifun == 0) || (M_icode == IRET && M_ifun == 0));

# Should I stall or inject a bubble into Pipeline Register D?
# At most one of these can be true.
bool D_stall = 
	# Conditions for a load/use hazardi
	 (E_icode == IPOPL || !(D_icode in {IOPL, IPUSHL}) && E_icode == IMRMOVL) &&
	 E_dstM in { d_srcA, d_srcB };

bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && !(E_ifun == 1 && M_icode == IRMMOVL)) ||
	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	!((E_icode == IPOPL || !(D_icode in {IOPL, IPUSHL}) && E_icode == IMRMOVL) && E_dstM in { d_srcA, d_srcB }) &&
	((D_icode == IRET && D_ifun == 0) || (E_icode == IRET && E_ifun == 0) || (M_icode == IRET && M_ifun == 0));
