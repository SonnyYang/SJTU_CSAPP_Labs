...
int bitMask(int highbit, int lowbit) {
  return ((2<<highbit)+(~(1<<lowbit)+1))&(~((highbit+(~lowbit)+1)>>31));
}
/* 
...
int isLess(int x, int y) {
  int mask = (x^y)>>31;
  return ((!(((y>>1)+(~(x>>1))+1)>>31))&mask) + ((!!((x+(~y)+1)>>31))&(~mask));
}
/* 
...
int isPower2(int x) {
  int neg_x = (~x)+1;
  int spec = ((x<<1)|(~(x<<1)+1))>>31;
			//return 0 for T_min or zero and -1 for others
  return !((x & neg_x)+neg_x) +(~spec);
}
/* 
...
int reverseBytes(int x) {
  ...
  int mask3 = 0xFF&(x>>16);
  int mask4 = 0xFF&(x>>24);
  return (mask1<<24)|(mask2<<16)|(mask3<<8)|mask4;
}
/* 
