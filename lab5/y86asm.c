...
{    
	...
	char* ntmp = (char *)malloc(sizeof(char) * strlen(name) +1);
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	tmp = (char *)malloc(sizeof(char)*(tail - head+1));
	memset(tmp,'\0',tail-head+1);  //important, avoid rubbish memory
...
parse_t parse_digit(char **ptr, long *value)
{
	...
	while(IS_DIGIT(tail) || (*tail >='a' && *tail <='f') || (*tail >= 'A'&& *tail <= 'F')|| *tail == 'x')
		tail++;

    /* calculate the digit, (NOTE: see strtoll()) */
	tmp = strtoll(head, &tail, 0);
      ...
      case I_DIRECTIVE: {
        ...
        switch (LOW(inst->code)) {
          case D_DATA: {    /* .long data - e.g., .long 0xC0 */
		  	ret = parse_data(&cur, &label, &value);
			if(ret == PARSE_ERR){
          ...
          case D_ALIGN: {   /* .align D - e.g., .align 4 */
			...
			y86bin->addr = vmaddr;
			y86bin->codes[1] = ((char*)&btmp)[0];
			y86bin->codes[2] = ((char*)&btmp)[1];
			y86bin->codes[3] = ((char*)&btmp)[2];
			y86bin->codes[4] = ((char*)&btmp)[3];
        ...
        /* relocate y86bin according itype */
			if(stmp){
				...
				case I_JMP: case I_CALL:
					rtmp->y86bin->codes[1] = ((char*)&stmp->addr)[0];
					rtmp->y86bin->codes[2] = ((char*)&stmp->addr)[1];
					rtmp->y86bin->codes[3] = ((char*)&stmp->addr)[2];
					rtmp->y86bin->codes[4] = ((char*)&stmp->addr)[3];
					break;

				case I_IRMOVL:
					rtmp->y86bin->codes[2] = ((char*)&stmp->addr)[0];
					rtmp->y86bin->codes[3] = ((char*)&stmp->addr)[1];
					rtmp->y86bin->codes[4] = ((char*)&stmp->addr)[2];
					rtmp->y86bin->codes[5] = ((char*)&stmp->addr)[3];
					break;
	
				default:
					rtmp->y86bin->codes[0] = ((char*)&stmp->addr)[0];
					rtmp->y86bin->codes[1] = ((char*)&stmp->addr)[1];
					rtmp->y86bin->codes[2] = ((char*)&stmp->addr)[2];
					rtmp->y86bin->codes[3] = ((char*)&stmp->addr)[3];
					break;
				}
			}
			else{
...
int binfile(FILE *out)
{
	...
	while(tmp != NULL){
		...
		int i = 0;
		for(;i < 6; i++)
			codes[i] = tmp->y86bin.codes[i];

		if( HIGH(codes[0]) == I_DIRECTIVE && LOW(codes[0]) != D_DATA){
			int need_write = 0;
			line_t *tmp_two = tmp;
			while(tmp_two != y86bin_listtail){
				tmp_two = tmp_two->next;
				if(tmp_two->type == TYPE_INS){
					need_write = 1;
					break;
				}
			}
			if(need_write){
				while(vmaddr != tmp->y86bin.addr){
					fwrite(&ctmp, sizeof(char), 1, out);
					vmaddr++;
				}
			}
		}
    	...
    	/* binary write y86 code to output file (NOTE: see fwrite()) */
			...
			fwrite(&codes, sizeof(char) * tmp->y86bin.bytes, 1, out);
		
		vmaddr += tmp->y86bin.bytes;
...
void print_line(line_t *line)
{
    ...
    if (line->type == TYPE_INS || line->type == TYPE_LAB) {
